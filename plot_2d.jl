...
function sample_2d_plot(shape, desired_samples; max_retry_times=100)
    p = scatter(leg=false, xlims=(0, shape[1]), ylims=(0, shape[2]))
    anim = Animation()
    R = √2
    grid = fill(-1, shape...)
                ...
                if grid[index] == -1
                    ...
                    if !collision
                        ...
                        if length(samples) % 100 == 0
                            scatter!(p, [x[1] for x in samples[end-99:end]], [x[2] for x in samples[end-99:end]], markersize=0.1)
                            frame(anim)
                        end
...
anim = sample_2d_plot((400, 400), 50000)
gif(anim, "poisson_2d.gif")
